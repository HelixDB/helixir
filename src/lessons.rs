...
pub fn get_lesson(lesson_id: usize) -> Lesson {
    match lesson_id {
        ...
        9 => Lesson {
            ...
            instructions: "Now that we know how to create nodes and their relationships, we need to be able to retrieve nodes from our graph. The simplest way is to retrieve nodes when we know their ID. Write 3 queries to get Continent (getContinent), Country (getCountry), and City (getCity) by node ID.".into(),
            hints: vec!["Add this header into your query.hx: QUERY embedDescription (city_id: ID, vector: [F64]) =>".into()],
            schema_answer: Some("lesson_answers/lesson4_schema.hx".into()),
        ...
        10 => Lesson {
            ...
            instructions: "In addition to retrieving nodes by ID, we often want to retrieve all nodes of a certain type. Since we have a hierarchical structure, we will also want to get all countries within a continent and all cities within a country.\n\nWrite 3 queries to get all **Continent** (getAllContinents), **Country** (getAllCountries), **City** nodes (getAllCities), 2 queries to get all **Country** (getCountriesInContinent) and **City** (getCitiesInContinent) nodes by their parent IDs (you can use **Out** for this after getting parent nodes from their ID".into(),
            hints: vec!["Add this header into your query.hx: QUERY embedDescription (city_id: ID, vector: [F64]) =>".into()],
            schema_answer: Some("lesson_answers/lesson4_schema.hx".into()),
